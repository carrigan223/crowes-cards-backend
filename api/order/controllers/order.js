...
/**
 * Given a dollar amount return number in cents
 * @param {number} number
 */
...
module.exports = {
  /**
   * Only returns orders that belong to that user
   * @param {any} ctx
   */
  async find(ctx) {
  ...
  /**
   * creates an order and sets up stripe checkout session for the front end
   * @param {any} ctx
   */
  async create(ctx) {
    ...
    const BASE_URL = ctx.request.headers.origin || "http://localhost:300";
  ...
  /**
   * Given a checkout_session, verifies payment and updates the order
   * @param {any} ctx
   */
  ...
  async confirm(ctx) {
    ...
    if (session.payment_status === "paid") {
      ...
      ctx.throw(400, "The Payment Wasnt Succseful, Please Contact Support");
    }
  },
};
